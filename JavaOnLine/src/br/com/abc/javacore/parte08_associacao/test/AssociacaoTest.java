...
public class AssociacaoTest
{
	...
	{
		Aluno aluno = new Aluno("Alexsandro", 35); // variável de referência do tipo Aluno referenciando um objeto do
													// tipo aluno.
		Aluno aluno2 = new Aluno("Joanna Darc", 30);
		// Deverão ser feitos de forma separadas para que haja a associação entre eles,
		// já que não tempo a certeza de que uma instância existe em outra classe.
		Seminario seminario = new Seminario("Como ser rico com programação");
		aluno.setSeminario(seminario); // Se parâmetro não contiver valores, dá uma exceção em tempo de execução -
										// java.lang.NullPointerException - tentando acessar um objeto que não existe
